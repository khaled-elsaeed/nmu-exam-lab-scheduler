    ...
    public function store(Request $request)
{
    ...
    try {
        ...
        if (Course::whereRaw('LOWER(code) = ?', [strtolower($validated['course_code'])])
            ->where('faculty_id', $facultyId)
            ->exists()) {
            return response()->json(['message' => 'Course code already exists in this faculty!'], 400);
        }

        if (Course::whereRaw('LOWER(name) = ?', [strtolower($validated['course_name'])])
            ->where('faculty_id', $facultyId)
            ->exists()) {
            return response()->json(['message' => 'Course name already exists in this faculty!'], 400);
        }
